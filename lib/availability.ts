...
export async function availabilityWarnings(p: {
  ...
  // dostępność nieskonfigurowana → nie ostrzegamy
  ...
  if (dayRows.length === 0) {
    return [`Korepetytor zwykle nie pracuje w ${WEEKDAY_NAMES[weekday].toLowerCase()}`]
  }
  ...
  if (!inside) {
    const ranges = dayRows
      ...
      .join(', ')
    return [`Korepetytor zwykle pracuje w ${WEEKDAY_NAMES[weekday].toLowerCase()}: ${ranges}`]
  }
